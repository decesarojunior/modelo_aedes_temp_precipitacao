setwd("C:\\Users\\deces\\Desktop\\Mestrado\\MCA022\\")
dados <- read.csv("PF-TMed-Precip.csv", sep=";", head=T)
dados$Data <- as.Date(dados$Data,"%d/%m/%Y")
dta.sum <- aggregate(x = dados[c("Precipitacao","Temp.Comp.Media")],
                     FUN = sum,
                     by = list(Group.date = dados$Data), na.rm=TRUE)

# Considerando uma janela de 10 dias e uma evaporacao de 10% a cada dia.
# essa funcao ira pegar a coluna Precipitacao do arquivo PF-TMed-Precip.csv
# entao sera realizado um somatorio dos dados da coluna precipitacao descontando a 
# precipitacao diaria.

totalChuva <- function(dia){
  sum(dta.sum$Precipitacao[(dia-min(9,dia-1)):dia]*seq(max(0.1,1-((dia-1)/10)),1,0.1)) 
}
...
ibm_aedes = new("indbasedModel3",
                ...
                equations = list(
                  ...
                  egg = function(inds, parms){
                    with(parms,{
                      ...
                      inds[inds$phase==1 & totalChuva(time) >= 10,'gdd'] <- inds[inds$phase==1 & totalChuva(time) >= 10,'gdd'] + (parms$temp[time]-parms$bt[1])
                      #checks if water level is at least 10mm
                      
                      inds[inds$phase==1 & totalChuva(time) >=10, 'age'] <- inds[inds$phase==1 & totalChuva(time) >=10, 'age'] + DELTAT
                      #if water < 10mm egg age won't change
                  ...
                  larva = function(inds, parms){
                    with(parms,{
                      if(totalChuva(time) >= 2){
                        inds[inds$phase==2, 'age'] <- inds[inds$phase==2, 'age'] + DELTAT 
                        
                        inds[inds$phase==2,'gdd'] <- inds[inds$phase==2,'gdd'] + (parms$temp[time]-parms$bt[2])
                        
                        LtP  <- which(inds$phase == 2 & inds$gdd>=parms$k[2]) #Larva to Pupa
                        if(length(LtP)>0) {
                          inds$phase[LtP] = 3 #change phase
                          inds$gdd[LtP] = inds$gdd[LtP] - parms$k[2]  
                          #inds who evolved will most likely surpass the minimum gdd necessary to next phase in this case, 
                          #we need to add the excess to the next stage 
                        }
                        inds
                      }
                      else{
                        inds<-subset(inds,inds$phase != 2)
                      }
                  ...
                  pupa = function(inds, parms){
                    with(parms,{
                      if(totalChuva(time) >= 2){
                        ...
                        inds[inds$phase==3,'gdd'] <- inds[inds$phase==3,'gdd'] + (parms$temp[time] - parms$bt[3])
                        ...
                        inds<-subset(inds,inds$phase != 3)
                      }
                    })},
                  adult = function(inds, parms){
                    with(parms,{
                      inds[inds$phase==4, 'age'] <- inds[inds$phase==4, 'age'] + DELTAT 
                      inds[inds$phase==4, 'gdd'] <- inds[inds$phase==4, 'gdd'] + (parms$temp[time] - parms$bt[4])
                      inds
                    })},
                  ...
                  survive  = function(inds, parms){ 
                    ...
                    eggsLive <-  subset(inds,inds$phase==1 & runif(length(age)) > (gdd*parms$deathrate[1]))  #selects
                    larvaLive <-  subset(inds,inds$phase==2 & runif(length(age)) > (gdd*parms$deathrate[2]))
                    pupaLive <-  subset(inds,inds$phase==3 & runif(length(age)) > (gdd*parms$deathrate[3]))
                    young <- subset(inds, inds$gdd < parms$k[4])
                    adultLive <-  subset(young,young$phase==4 & runif(length(age)) > (gdd*parms$deathrate[4])-0.2)
                    
                    inds <- rbind(eggsLive,larvaLive,pupaLive,adultLive)
                    # print(dta.sum$Temp.Comp.Media[time])
                    inds
                    
                  }
                ),parms = list(
                  maxage      = 44,       #days
                  temp        = dta.sum$Temp.Comp.Media, #runif(1,25,40), 
                  water = 10, #runif(1,8,10),
                  bt = c(2.9, 8.6, 10.8, 3.43),
                  k = c(305.8, 433.5, 465.6, 800), #added a new parameter (800) for the adult mosquitos 
                  deathrate = c(0.383, 0.075, 0.062, 0.02)/ c(305.8, 433.5, 465.6, 800)
                ),   
                # print(dta.sum$Temp.Comp.Media[parms$DELTAT]),
                ...
                times = c(from=0, to=360, by=1),
                solver = "myiteration"
)



#adicionar a temp e chuva de Passo Fundo em 2017
#fazer a morte das larvas e pupas caso o nivel de agua seja inferior que 2mm
...
##-------------------------------------------------------------------
setMethod("plot", c("indbasedModel3", "missing"), function(x, y, ...) {
